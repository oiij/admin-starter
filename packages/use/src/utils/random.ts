...
/** 随机ip */
const randomIp = (): string => Array.from({ length: 4 }).fill(0).map((_, i) => Math.floor(Math.random() * 255) + (i === 0 ? 1 : 0)).join('.')
/** 随机字符串 */
const generateString = (length: number, chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ') => Array.from({ length }).fill('').map(() => chars[Math.floor(Math.random() * chars.length)]).join('')
/** 随机符号 */
