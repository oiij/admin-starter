...
export function useNaiveTheme(darkMode?: ComputedRef<boolean>, language?: ComputedRef<string>) {
  ...
  const locale = computed(() => naiveLocaleMap[language?.value ?? 'zh-CN'].locale)
  const dateLocale = computed(() => naiveLocaleMap[language?.value ?? 'zh-CN'].dateLocale)
