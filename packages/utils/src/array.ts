...
/** 在给定范围内创建数字数组 */
const range = (min: number, max: number): number[] => [...Array.from({ length: max - min + 1 }).keys()].map(i => i + min)
/** 清空数组 */
...
/** 生成字母字符数组 */
const alphabet = (): string[] => Array.from({ length: 26 }).fill(0).map((_, i) => String.fromCharCode(i + 97))
/** 获取数组的平均值 */
...
/** 元素之间穿插元素 */
const intersperse = <T>(a: T[], s: T): T[] => [...Array.from({ length: 2 * a.length - 1 })].map((_, i) => (i % 2 ? s : a[i / 2]))
/** 根据条件对阵列进行分区 */
// eslint-disable-next-line no-sequences
...
/** 重复一个数组 */
const repeat = <T, _>(arr: T[], n: number): T[] => Array.from<T[]>({ length: n }).fill(arr).flat()
/** 随机播放数组 */
