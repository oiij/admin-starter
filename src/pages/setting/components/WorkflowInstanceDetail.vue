...
const { defaultValues } = defineProps<{
  defaultValues: WorkflowInstanceType['Doc']
}>()
...
<template>
  <div class="h-[600px] w-[600px] flex-col gap-[10px]">
    <NSteps>
      <NStep v-for="item in defaultValues.workflow.nodes" :key="item.id" :title="item.name" />
    </NSteps>
    ...
    <NFlex justify="end">
      <AccessValidator :value="_APPROVAL_ACCESS">
        <NButton type="success" :loading="approvalLoading" @click="() => handleApproval({ _id: defaultValues._id, action: 'APPROVED' })">
          同意
        </NButton>
      </AccessValidator>
      <AccessValidator :value="_APPROVAL_ACCESS">
        <NButton type="error" :loading="approvalLoading" @click="() => handleApproval({ _id: defaultValues._id, action: 'REJECTED' })">
          拒绝
        </NButton>
      </AccessValidator>
    </NFlex>
